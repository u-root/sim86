...
static void x86emuOp2_illegal_op(u8 op2)
{
    ...
    TRACE_REGS();
    loggy("%04x:%04x: %02X ILLEGAL EXTENDED X86 OPCODE!\n",
        CS, IP-2, op2);
    HALT_SYS();
    END_OF_INSTR();
}
...
static void x86emuOp2_long_jump(u8 op2)
{
    ...
    if (cond) {
        IP = (u16)target;
	JMP_TRACE(saved_cs, saved_ip, CS, IP, " LONG COND ");
    }
    DECODE_CLEAR_SEGOVR();
    END_OF_INSTR();
}
...
static void x86emuOp2_bswap(u8 op2)
{
    ...
    switch (op2) {
      case 0xc8:
        DECODE_PRINTF("EAX\n");
        EAX = x86emu_bswap(EAX);
        break;
      case 0xc9:
        DECODE_PRINTF("ECX\n");
        ECX = x86emu_bswap(ECX);
        break;
      case 0xca:
        DECODE_PRINTF("EDX\n");
        EDX = x86emu_bswap(EDX);
        break;
      case 0xcb:
        DECODE_PRINTF("EBX\n");
        EBX = x86emu_bswap(EBX);
        break;
      case 0xcc:
        DECODE_PRINTF("ESP\n");
        ESP = x86emu_bswap(ESP);
        break;
      case 0xcd:
        DECODE_PRINTF("EBP\n");
        EBP = x86emu_bswap(EBP);
        break;
      case 0xce:
        DECODE_PRINTF("ESI\n");
        ESI = x86emu_bswap(ESI);
        break;
      case 0xcf:
        DECODE_PRINTF("EDI\n");
        EDI = x86emu_bswap(EDI);
        break;
    }
    TRACE_AND_STEP();
    DECODE_CLEAR_SEGOVR();
...
static void x86emuOp2_shld_CL(u8 X86EMU_UNUSED(op2))
{
    ...
    if (mod < 3) {
        ...
        if (mode & SYSMODE_PREFIX_DATA) {
            ...
            TRACE_AND_STEP();
            destval = fetch_data_long(destoffset);
            destval = shld_long(destval,*shiftreg,CL);
            store_data_long(destoffset, destval);
        } else {
            u16 destval;
            u16 *shiftreg;
            ...
            TRACE_AND_STEP();
            destval = fetch_data_word(destoffset);
            destval = shld_word(destval,*shiftreg,CL);
            store_data_word(destoffset, destval);
        }
    } else {                     /* register to register */
        if (mode & SYSMODE_PREFIX_DATA) {
            ...
            DECODE_PRINTF(",CL\n");
            TRACE_AND_STEP();
            *destreg = shld_long(*destreg,*shiftreg,CL);
        } else {
            u16 *destreg,*shiftreg;
            ...
            DECODE_PRINTF(",CL\n");
            TRACE_AND_STEP();
            *destreg = shld_word(*destreg,*shiftreg,CL);
        }
    }
    DECODE_CLEAR_SEGOVR();
    END_OF_INSTR();
...
static void x86emuOp2_shrd_CL(u8 X86EMU_UNUSED(op2))
{
    ...
    if (mod < 3) {
        ...
        if (mode & SYSMODE_PREFIX_DATA) {
            ...
            TRACE_AND_STEP();
            destval = fetch_data_long(destoffset);
            destval = shrd_long(destval,*shiftreg,CL);
            store_data_long(destoffset, destval);
        } else {
            u16 destval;
            u16 *shiftreg;
            ...
            TRACE_AND_STEP();
            destval = fetch_data_word(destoffset);
            destval = shrd_word(destval,*shiftreg,CL);
            store_data_word(destoffset, destval);
        }
    } else {                     /* register to register */
        if (mode & SYSMODE_PREFIX_DATA) {
            ...
            DECODE_PRINTF(",CL\n");
            TRACE_AND_STEP();
            *destreg = shrd_long(*destreg,*shiftreg,CL);
        } else {
            u16 *destreg,*shiftreg;
            ...
            DECODE_PRINTF(",CL\n");
            TRACE_AND_STEP();
            *destreg = shrd_word(*destreg,*shiftreg,CL);
        }
    }
    DECODE_CLEAR_SEGOVR();
    END_OF_INSTR();
...
static void x86emuOp2_lss_R_IMM(u8 X86EMU_UNUSED(op2))
{
    ...
    if (mod < 3) {
        ...
        *dstreg = fetch_data_word(srcoffset);
        SS = fetch_data_word(srcoffset + 2);
    } else {                     /* register to register */
        /* UNDEFINED! */
        TRACE_AND_STEP();
    }
...
static void x86emuOp2_lfs_R_IMM(u8 X86EMU_UNUSED(op2))
{
    ...
    if (mod < 3) {
        ...
        TRACE_AND_STEP();
        *dstreg = fetch_data_word(srcoffset);
        FS = fetch_data_word(srcoffset + 2);
    } else {                     /* register to register */
        /* UNDEFINED! */
        TRACE_AND_STEP();
    }
...
static void x86emuOp2_lgs_R_IMM(u8 X86EMU_UNUSED(op2))
{
    ...
    if (mod < 3) {
        ...
        TRACE_AND_STEP();
        *dstreg = fetch_data_word(srcoffset);
        GS = fetch_data_word(srcoffset + 2);
    } else {                     /* register to register */
        /* UNDEFINED! */
        TRACE_AND_STEP();
    }
...
static void x86emuOp2_btX_I(u8 op2)
{
    ...
    default:
        DECODE_PRINTF("ILLEGAL EXTENDED X86 OPCODE\n");
        TRACE_REGS();
        loggy("%04x:%04x: %02X%02X ILLEGAL EXTENDED X86 OPCODE EXTENSION!\n",
                CS, IP-3,op2, (mod<<6)|(rh<<3)|rl);
        HALT_SYS();
    }
    if (mod < 3) {

...
func x86emuOp_two_byte(_ uint8) {
	op2 := sys_rdb((uint32(M.x86.seg.CS.Get()) << 4) + (M.x86.R_IP))
	M.x86.R_IP++
...
var cond int32
    ...
    if (cond) {
        M.x86.R_IP = target;
 JMP_TRACE(M.x86.saved_cs, M.x86.saved_ip, M.x86.seg.CS.Get(), M.x86.R_IP, " NEAR COND ");
    }
    DECODE_CLEAR_SEGOVR();
    END_OF_INSTR();
}
...
M.X86.C.Dec()
            M.x86.DI.Change(inc)
            if (ACCESS_FLAG(F_ZF)) {break;} /* zero flag set means equal */
            if (M.x86.intr & INTR_HALTED){ break;}
        }
...
func x86emuOp_ret_near_IMM(_ uint8) {
 ...
 RETURN_TRACE(M.x86.saved_cs,M.x86.saved_ip, M.x86.seg.CS.Get(), M.x86.R_IP, "NEAR");
	M.x86.SP.Add( imm)
    DECODE_CLEAR_SEGOVR();
    END_OF_INSTR();
}
...
func x86emuOp_ret_far_IMM(_ uint8) {
 ...
 RETURN_TRACE(M.x86.saved_cs,M.x86.saved_ip, M.x86.seg.CS.Get(), M.x86.R_IP, "FAR");
    M.x86.R_SP += imm;
    DECODE_CLEAR_SEGOVR();
    END_OF_INSTR();
}
...
var offset int8
    ...
    if (M.x86.R_CX == 0) {
        M.x86.R_IP = target;
 JMP_TRACE(M.x86.saved_cs, M.x86.saved_ip, M.x86.seg.CS.Get(), M.x86.R_IP, " CXZ ");
    }
    DECODE_CLEAR_SEGOVR();
    END_OF_INSTR();
}
...
var destval32 uint32
    ...
    if (mod < 3) {
        ...
        case 0: /* inc word ptr ... */
            if (M.x86.mode & SYSMODE_PREFIX_DATA) != 0 {
                destval32 = fetch_data_long(uint16(destoffset));
                TRACE_AND_STEP();

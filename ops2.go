...
func x86emuOp2_rdtsc(_ uint8) {
	...
	S(EAX, uint32(counter))
	S(EDX, uint32(counter >> 32))
	DECODE_CLEAR_SEGOVR()
	END_OF_INSTR()
}
...
func x86emuOp2_long_jump(op2 uint8) {
	...
	if cond {
		S16(IP, uint16(target))
		JMP_TRACE(M.x86.saved_cs, M.x86.saved_ip, G16(CS), G16(IP), " LONG COND ")
	}
	DECODE_CLEAR_SEGOVR()
	END_OF_INSTR()
}
...
func x86emuOp2_bswap(op2 uint8) {
	...
	case 0xc8:
		DECODE_PRINTF("EAX\n")
		S(EAX, x86emu_bswap(G32(EAX)))
		break
	case 0xc9:
		DECODE_PRINTF("ECX\n")
		S(ECX, x86emu_bswap(G32(ECX)))
		break
	case 0xca:
		DECODE_PRINTF("EDX\n")
		S(EDX, x86emu_bswap(G32(EDX)))
		break
	case 0xcb:
		DECODE_PRINTF("EBX\n")
		S(EBX, x86emu_bswap(G32(EBX)))
		break
	case 0xcc:
		DECODE_PRINTF("ESP\n")
...
func x86emuOp2_shld_IMM(_ uint8) {
	...
	if mod < 3 {
		...
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			destval = shld_long(destval, G32(shiftreg), shift)
			...
			destval = shld_word(destval, G16(shiftreg), shift)
		...
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			S32(destreg, shld_long(G32(destreg), G32(shiftreg), shift))
		} else {
			...
			S16(destreg, shld_word(G16(destreg), G16(shiftreg), shift))
		}
	}
	DECODE_CLEAR_SEGOVR()
	END_OF_INSTR()
...
func x86emuOp2_shld_CL(_ uint8) {
	...
	if mod < 3 {
		...
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			destval := fetch_data_long(destoffset)
			destval = shld_long(destval, G32(shiftreg), G8(CL))
			store_data_long(destoffset, destval)
		} else {
			...
			destval := fetch_data_word(destoffset)
			destval = shld_word(destval, G16(shiftreg), G8(CL))
			store_data_word(destoffset, destval)
		}
	} else { /* register to register */
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			S32(destreg, shld_long(G32(destreg), G32(shiftreg), G8(CL)))
		} else {
			...
			S16(destreg, shld_word(G16(destreg), G16(shiftreg), G8(CL)))
		}
	}
	DECODE_CLEAR_SEGOVR()
	END_OF_INSTR()
...
func x86emuOp2_bts_R(_ uint8) {
	...
	if mod < 3 {
		...
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			store_data_long(srcoffset+uint32(disp), srcval|mask)
		} else {
			...
			mask := uint16(0x1 << bit)
			CONDITIONAL_SET_FLAG(srcval&mask, F_CF)
			store_data_word(srcoffset+uint32(disp), srcval|mask)
		}
	} else { /* register to register */
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
...
func x86emuOp2_shrd_IMM(_ uint8) {
	...
	if mod < 3 {
		...
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			destval := fetch_data_long(destoffset)
			destval = shrd_long(destval, G32(shiftreg), shift)
			store_data_long(destoffset, destval)
		} else {
			...
			destval := fetch_data_word(destoffset)
			destval = shrd_word(destval, G16(shiftreg), shift)
			store_data_word(destoffset, destval)
		}
	} else { /* register to register */
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			S32(destreg, shrd_long(G32(destreg), G32(shiftreg), shift))
		} else {
			...
			S16(destreg, shrd_word(G16(destreg), G16(shiftreg), shift))
		}
	}
	DECODE_CLEAR_SEGOVR()
	END_OF_INSTR()
...
func x86emuOp2_shrd_CL(_ uint8) {
	...
	if mod < 3 {
		...
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			destval := fetch_data_long(destoffset)
			destval = shrd_long(destval, G32(shiftreg), G8(CL))
			store_data_long(destoffset, destval)
		} else {
			...
			destval := fetch_data_word(destoffset)
			destval = shrd_word(destval, G16(shiftreg), G8(CL))
			store_data_word(destoffset, destval)
		}
	} else { /* register to register */
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			S32(destreg, shrd_long(G32(destreg), G32(shiftreg), G8(CL)))
		} else {
			...
			S16(destreg, shrd_word(G16(destreg), G16(shiftreg), G8(CL)))
		}
	}
	DECODE_CLEAR_SEGOVR()
	END_OF_INSTR()
...
func x86emuOp2_imul_R_RM(_ uint8) {
	...
	if mod < 3 {
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			imul_long_direct(&res_lo, &res_hi, G32(destreg), srcval)
			if res_hi != 0 {
				SET_FLAG(F_CF)
				SET_FLAG(F_OF)
			} else {
			...
			res := uint32(G(destreg)) * uint32(srcval)
			if res > 0xFFFF {
				SET_FLAG(F_CF)
				SET_FLAG(F_OF)
			} else {
		...
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			imul_long_direct(&res_lo, &res_hi, G32(destreg), G32(srcreg))
			if res_hi != 0 {
				SET_FLAG(F_CF)
				SET_FLAG(F_OF)
			} else {
			...
			DECODE_PRINTF(",")
			srcreg := decode_rm_word_register(rl)
			res := uint32(G(destreg)) * uint32(G(srcreg))
			if res > 0xFFFF {
				SET_FLAG(F_CF)
				SET_FLAG(F_OF)
			} else {
...
func x86emuOp2_lss_R_IMM(_ uint8) {
	...
	if mod < 3 {
		...
		S16(SS, fetch_data_word(srcoffset + 2))
	} else { /* register to register */
		/* UNDEFINED! */
		if TRACE_AND_STEP() {
			END_OF_INSTR()
...
func x86emuOp2_lfs_R_IMM(_ uint8) {
	...
	if mod < 3 {
		...
		S(dstreg,fetch_data_word(srcoffset))
		S16(FS, fetch_data_word(srcoffset + 2))
	} else { /* register to register */
		/* UNDEFINED! */
		if TRACE_AND_STEP() {
			END_OF_INSTR()
...
func x86emuOp2_lgs_R_IMM(_ uint8) {
	...
	if mod < 3 {
		...
		S(dstreg,fetch_data_word(srcoffset))
		S16(GS, fetch_data_word(srcoffset + 2))
	} else { /* register to register */
		/* UNDEFINED! */
		if TRACE_AND_STEP() {
			END_OF_INSTR()
...
func x86emuOp2_btX_I(op2 uint8) {
	...
	default:
		...
		fmt.Printf("%04x:%04x: %02X%02X ILLEGAL EXTENDED X86 OPCODE EXTENSION!\n",
			G16(CS), G16(IP)-3, op2, (mod<<6)|(rh<<3)|rl)
		HALT_SYS()
	}
	if mod < 3 {
...
func x86emuOp2_bsf(_ uint8) {
	...
	if mod < 3 {
		...
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			for S(dstreg,0); G(dstreg) < 32; dstS(reg,dstG(reg) + 1) {
			...
			srcval := fetch_data_word(srcoffset)
			CONDITIONAL_SET_FLAG(srcval == 0, F_ZF)
			for S(dstreg,0); G(dstreg) < 16; dstS(reg,dstG(reg) + 1) {
				if ((srcval >> G(dstreg)) & 1) != 0 {
					break
				}
			}
...
func x86emuOp2_bsr(_ uint8) {
	...
	if mod < 3 {
		...
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			srcval := fetch_data_long(srcoffset)
			CONDITIONAL_SET_FLAG(srcval == 0, F_ZF)
			for S(dstreg,31); G(dstreg) > 0; dstS(reg,dstG(reg) - 1) {
				if ((srcval >> G(dstreg)) & 1) != 0 {
					break
				}
			}
			...
			srcval := fetch_data_word(srcoffset)
			CONDITIONAL_SET_FLAG(srcval == 0, F_ZF)
			for S(dstreg,15); G(dstreg) > 0; dstS(reg,dstG(reg) - 1) {
				if ((srcval >> G(dstreg)) & 1) != 0 {
					break
				}
			}
...
func x86emuOp2_movsx_byte_R_RM(_ uint8) {
	...
	if mod < 3 {
		if M.x86.mode&SYSMODE_PREFIX_DATA != 0 {
			...
			DECODE_PRINTF(",")
			srcoffset := decode_rmXX_address(mod, rl)
			srcval := uint32(int8(fetch_data_byte(srcoffset)))
			DECODE_PRINTF("\n")
			if TRACE_AND_STEP() {
				END_OF_INSTR()
				return
			...
			DECODE_PRINTF(",")
			srcoffset := decode_rmXX_address(mod, rl)
			srcval := uint16(int8(fetch_data_byte(srcoffset)))
			DECODE_PRINTF("\n")
			if TRACE_AND_STEP() {
				END_OF_INSTR()
				return
...
func x86emuOp2_movsx_word_R_RM(_ uint8) {
	...
	if mod < 3 {
		...
		DECODE_PRINTF(",")
		srcoffset := decode_rmXX_address(mod, rl)
		srcval := uint32(int16(fetch_data_word(srcoffset)))
		DECODE_PRINTF("\n")
		if TRACE_AND_STEP() {
			END_OF_INSTR()
			return

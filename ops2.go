...
func x86emuOp2_bswap(op2 u8) {
    ...
    switch (op2) {
      ...
      case 0xcc:
        DECODE_PRINTF("ESP\n");
        M.x86.gen.SP.Set32(x86emu_bswap(M.x86.gen.SP.Get32()))
        break;
      case 0xcd:
        DECODE_PRINTF("EBP\n");
        M.x86.gen.BP.Set32(x86emu_bswap(M.x86.gen.BP.Get32()))
        break;
      case 0xce:
        DECODE_PRINTF("ESI\n");
        M.x86.gen.SI.Set32(x86emu_bswap(M.x86.gen.SI.Get32()))
        break;
      case 0xcf:
        DECODE_PRINTF("EDI\n");
        M.x86.gen.DI.Set32(x86emu_bswap(M.x86.gen.DI.Get32()))
        break;
    }
    TRACE_AND_STEP();
    DECODE_CLEAR_SEGOVR();
...
func x86emuOp2_shld_IMM(_ u8) {
    ...
    if (mod < 3) {
        ...
        if (M.x86.mode & SYSMODE_PREFIX_DATA) {
            ...
            DECODE_PRINTF2("%d\n", shift);
            TRACE_AND_STEP();
            destreg.Set(shld_long(destreg.Get(),*shiftreg,shift))
        } else {
            ...
            DECODE_PRINTF2("%d\n", shift);
            TRACE_AND_STEP();
            destreg.Set(shld_word(destreg.Get(),*shiftreg,shift))
        }
    }
    DECODE_CLEAR_SEGOVR();
    END_OF_INSTR();
...
func x86emuOp2_shld_CL(_ u8) {
    ...
    if (mod < 3) {
        ...
        if (M.x86.mode & SYSMODE_PREFIX_DATA) {
            ...
            DECODE_PRINTF(",CL\n");
            TRACE_AND_STEP();
            destreg.Set(shld_long(destreg.Get(),*shiftreg,M.x86.R_CL))
        } else {
            ...
            DECODE_PRINTF(",CL\n");
            TRACE_AND_STEP();
            destreg.Set(shld_word(destreg.Get(),*shiftreg,M.x86.R_CL))
        }
    }
    DECODE_CLEAR_SEGOVR();
    END_OF_INSTR();
...
func x86emuOp2_shrd_IMM(_ u8) {
    ...
    if (mod < 3) {
        ...
        if (M.x86.mode & SYSMODE_PREFIX_DATA) {
            ...
            DECODE_PRINTF2("%d\n", shift);
            TRACE_AND_STEP();
            destreg.Set(shrd_long(destreg.Get(),*shiftreg,shift))
        } else {
            ...
            DECODE_PRINTF2("%d\n", shift);
            TRACE_AND_STEP();
            destreg.Set(shrd_word(destreg.Get(),*shiftreg,shift))
        }
    }
    DECODE_CLEAR_SEGOVR();
    END_OF_INSTR();
...
func x86emuOp2_shrd_CL(_ u8) {
    ...
    if (mod < 3) {
        ...
        if (M.x86.mode & SYSMODE_PREFIX_DATA) {
            ...
            DECODE_PRINTF(",CL\n");
            TRACE_AND_STEP();
            destreg.Set(shrd_long(destreg.Get(),*shiftreg,M.x86.R_CL))
        } else {
            ...
            DECODE_PRINTF(",CL\n");
            TRACE_AND_STEP();
            destreg.Set(shrd_word(destreg.Get(),*shiftreg,M.x86.R_CL))
        }
    }
    DECODE_CLEAR_SEGOVR();
    END_OF_INSTR();
...
func x86emuOp2_imul_R_RM(_ u8) {
    ...
    if (mod < 3) {
        if (M.x86.mode & SYSMODE_PREFIX_DATA) {
            ...
            TRACE_AND_STEP();
            imul_long_direct(&res_lo,&res_hi,(s32)destreg.Get(),int32(srcval));
            if (res_hi != 0) {
                SET_FLAG(F_CF);
                SET_FLAG(F_OF);
            } else {
        ...
        if (M.x86.mode & SYSMODE_PREFIX_DATA) {
            ...
            srcreg := DECODE_RM_LONG_REGISTER(rl);
            TRACE_AND_STEP();
            imul_long_direct(&res_lo,&res_hi,(s32)destreg.Get(),(s32)srcreg.Get());
            if (res_hi != 0) {
                SET_FLAG(F_CF);
                SET_FLAG(F_OF);
            } else {
